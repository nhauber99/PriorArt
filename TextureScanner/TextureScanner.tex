...
\begin{enumerate}
	...
	\item \textbf{Simplified Scanner:} A simpler scanner design with fewer light sources and potentially without polarization could be realized. To handle the reduced data, a neural network like a U-Net could be trained to predict the different textures. The network training could ideally be done using the results from a more complex scanner as ground truth. The input during training could be renders of the ground truth or a subset of images captured by the larger scanner. The software could also be used as its own product. This could especially be useful for existing scanners which employ photometric stereo in addition to photogrammetry and normally have fewer light sources.
	...
	\item \textbf{Single shot spectral multiplexed and polarization multiplexed photometric stereo:} Using modern dual pixel sensors (like the modern Canon Cameras using DPAF) one can use both spectral multiplexing (\href{https://patents.google.com/patent/US20120056994}{Patent US20120056994A1}) in combination with having two polarization filters with orthogonal polarization on both halves of the lens (similar to the concept used in \href{https://arxiv.org/pdf/2110.05655}{Defocus Map Estimation and Deblurring from a Single Dual-Pixel Image}. By doing this one can have a total of 6 light sources with 3 different spectra for each of the color channels of the camera and 2 different polarizations. This would work best for metallic objects, because they don't have a diffuse reflectance and cross polarization will filter out all reflected light, will parallel polarization will filter none of it, meaning a single image captured like this can be separated into 6 different channels.
\end{enumerate}
